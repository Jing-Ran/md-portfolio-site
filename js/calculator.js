(function() {
  ...
  function fixNums(value) {
    return value.replace(/[\d.]+/g, (n) => parseFloat(n));
  }

  function displayValue(btn) {
    let lastInput = inputScreen.value.substr(-1);
  ...
  calculator.addEventListener('click', (e) => {
    if (e.target.classList.contains('display-btns')) {
      displayValue(e.target);
    }
  });

  // clearBtn: click once to clean inputScreen
  clearBtn.addEventListener('click', () => inputScreen.value = '');

  // clearBtn: double click to clean topScreen
  clearBtn.addEventListener('dblclick', () => topScreen.value = '');

  deleteBtn.addEventListener('click', () =>
    inputScreen.value =
      inputScreen.value.substring(0, inputScreen.value.length - 1)
  );

  equalBtn.addEventListener('click', () => {
    let fixedValue = fixNums(inputScreen.value);
